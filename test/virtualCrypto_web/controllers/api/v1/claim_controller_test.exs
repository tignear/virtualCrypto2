defmodule ClaimControllerTest.V1 do
  ...
  test "get user1 claim0", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.v1_claim_path(conn, :get_by_id, (claims |> at(0)).claim.id))
  ...
  test "get claim1 by payer", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.v1_claim_path(conn, :get_by_id, (claims |> at(1)).claim.id))
  ...
  test "get claim2 by claimant", %{conn: conn, claims: claims} = ctx do
    ...
    conn =
      get(
        conn,
        Routes.v1_claim_path(conn, :get_by_id, (claims |> approved_claim()).claim.id)
      )
  ...
  test "get claim4 by claimant", %{conn: conn, claims: claims} = ctx do
    ...
    conn =
      get(
        conn,
        Routes.v1_claim_path(conn, :get_by_id, (claims |> canceled_claim()).claim.id)
      )
  ...
  test "get claim by payer and claimant", %{conn: conn, claims: claims} = ctx do
    conn = set_user_auth(conn, :user, ctx.user1, ["vc.claim"])
    conn = get(conn, Routes.v1_claim_path(conn, :get_by_id, (claims |> at(5)).claim.id))
  ...
  test "patch claim0 with invalid status", %{conn: conn, claims: claims, user1: user1} do
    ...
    conn =
      patch(conn, Routes.v1_claim_path(conn, :patch, (claims |> at(0)).claim.id), %{
        "status" => "nyan!"
      })
  ...
  test "approve claim by claimant", %{conn: conn, claims: claims, user1: user1} do
    ...
    conn =
      patch(
        conn,
        Routes.v1_claim_path(conn, :patch, (claims |> at(0)).claim.id),
        %{"status" => "approved"}
      )
  ...
  test "approve claim by payer but not_enough_amount",
    ...
    conn =
      patch(
        conn,
        Routes.v1_claim_path(conn, :patch, (claims |> at(1)).claim.id),
        %{"status" => "approved"}
      )
  ...
  test "approve approved claim",
    ...
    conn =
      patch(
        conn,
        Routes.v1_claim_path(conn, :patch, (claims |> approved_claim()).claim.id),
        %{"status" => "approved"}
      )
